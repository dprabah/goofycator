    ...
    try:
        ...
        if tverify.verify_twitter_handle(twitter_handle):
            if verify_handle_already_added(db, twitter_handle, guild_name):
                db.insert({'handle': twitter_handle, 'verified': False, 'unique_id': uuid.uuid4().hex,
                           'user_id': str(ctx.message.author.id), 'guild_id': str(guild_name)})
...
def verify_handle_already_added(db, handle, guild_id):
    q = Query()
    return not len(db.search((q.handle == handle) & (q.guild_id == guild_id) & (q.verified == False))) > 0
...
def get_user_row(db, handle, unique_id):
    q = Query()
    return db.search((q.handle == handle) & (q.unique_id == unique_id) & (q.verified == False))


def update_processed_row(db, handle, unique_id, guild_id):
    q = Query()
    db.update({'verified': True}, (q.handle == handle) & (q.unique_id == unique_id) & (q.guild_id == guild_id))
