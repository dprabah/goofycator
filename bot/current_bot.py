...
async def insta_download(ctx, *args):
    ...
    try:
        ...
        extract_video_link = re.search('meta property="og:video" content=[\'"]?([^\'" >]+)', src)
...
async def convert(ctx):
    ...
    await ctx.send(embed=embed)
    try:
        cmd = ['ffmpeg', '-i', user_video, '-map', '0', '-c', 'copy', 'test.mp4']
        subprocess.run(cmd)
    except Exception as e:
        print(e)
    await ctx.reply(file=discord.File("test.mp4"))
